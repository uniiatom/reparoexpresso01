...
import {
  ArrowLeft, Loader2, Wrench, MapPin, Calendar, Star,
  CheckSquare, Image, User, Phone, ChevronDown, ChevronUp, X
} from 'lucide-react';
          ...
          <div className="flex justify-center gap-2 mt-4">
            {photos.map((_, i) => (
              <button key={i} onClick={() => setCurrent(i)}
                className={cn("w-2.5 h-2.5 rounded-full", i === current ? "bg-white" : "bg-white/40")} />
            ))}
          </div>
        )}
      </div>
...
function ProviderMiniCard({ providerId, providerName, providerPhone }) {
  const { data: provider } = useQuery({
    queryKey: ['provider-mini', providerId],
    queryFn: () => base44.entities.Provider.filter({ id: providerId }).then(r => r[0] || null),
    enabled: !!providerId,
  });
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      {provider && provider.photo_body_url && (
      ...
      <div className="flex-1 min-w-0">
        ...
        {provider && (
          <div className="flex items-center gap-1 mt-0.5">
            ...
            <span className="text-xs font-medium">{provider.rating ? provider.rating.toFixed(1) : '5.0'}</span>
...
function ServiceCard({ service }) {
  ...
  const allPhotos = [
    ...
    ...(service.checklist && service.checklist.photos ? service.checklist.photos : []),
    ...(service.additional_points
      ? service.additional_points.flatMap(p => p.photos || [])
      : []),
  ];

  const checklistItems = service.checklist && service.checklist.items ? service.checklist.items : [];
  ...
  return (
    <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
          ...
          {(service.final_price || service.estimated_price) && (
            <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-xl px-3 py-2">
              <p className="text-sm font-semibold text-green-800">
                Valor {service.final_price ? 'final' : 'estimado'}
              </p>
              <p className="text-lg font-black text-green-700">
                R$ {(service.final_price || service.estimated_price).toFixed(2)}
              </p>
            </div>
          )}
          ...
          {service.rating_client && (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex gap-0.5">
                {[1, 2, 3, 4, 5].map(i => (
                  <Star key={i} className={cn("w-4 h-4", i <= service.rating_client
                    ? "text-yellow-400 fill-yellow-400"
                    : "text-muted-foreground")} />
                ))}
              </div>
              {service.rating_comment && (
                <p className="text-xs text-muted-foreground italic w-full">"{service.rating_comment}"</p>
              )}
            </div>
          )}
              ...
              <div className="space-y-1.5">
                {checklistItems.map((item, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <p className={cn("text-sm", item.checked ? "text-foreground" : "text-muted-foreground")}>
                      {item.label}
                    </p>
                  </div>
                ))}
              </div>
              {service.checklist && service.checklist.notes && (
              ...
              <div className="space-y-2">
                {service.additional_points.map((pt, i) => (
                  <div key={i} className="bg-orange-50 border border-orange-200 rounded-xl p-3">
                    ...
                    {pt.extra_cost > 0 && (
                      <p className="text-xs font-bold text-orange-800 mt-1">+ R$ {pt.extra_cost.toFixed(2)}</p>
                    )}
                  </div>
                ))}
              </div>
      ...
      {lightbox !== null && (
        <PhotoLightbox photos={allPhotos} initialIndex={lightbox} onClose={() => setLightbox(null)} />
      )}
    </div>
  );
}
    ...
    <div className="min-h-screen bg-background max-w-lg mx-auto px-4 py-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-foreground">Minha Ficha</h1>
          <p className="text-xs text-muted-foreground">{user && user.full_name}</p>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-primary/10 to-primary/5 rounded-3xl p-5 border border-primary/20 mb-6">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1">
            <p className="font-black text-xl text-foreground">{user && user.full_name}</p>
            <p className="text-sm text-muted-foreground">{user && user.email}</p>
            {clientProfile && clientProfile.phone && (
              <div className="flex items-center gap-1 mt-0.5">
                <Phone className="w-3 h-3 text-muted-foreground" />
                <p className="text-xs text-muted-foreground">{clientProfile.phone}</p>
              </div>
            )}
            {clientProfile && clientProfile.referral_code && (
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-5">
        {['todos', 'concluido', 'em_andamento', 'aguardando', 'agendado', 'cancelado'].map(s => (
          <button key={s} onClick={() => setStatusFilter(s)}
            ...
            {s === 'todos'
              ? 'Todos (' + serviceRequests.length + ')'
              : (STATUS_CONFIG[s] ? STATUS_CONFIG[s].label : s) + ' (' + serviceRequests.filter(r => r.status === s).length + ')'}
          </button>
        ))}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-muted/50 rounded-2xl p-10 text-center border border-dashed border-border">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {statusFilter !== 'todos' ? 'Tente outro filtro' : 'Nenhum serviço solicitado ainda'}
          </p>
        </div>
      ) : (
